...
| `pas load [--source all\|cbr\|fedstat]` | Инкрементальная загрузка. ЦБ — с даты последнего курса минус 7 дней перекрытия. fedstat — обновление последнего года и догрузка кусков (год × товар), для которых в журнале нет успешного шага; не более 15 запросов за запуск. |
| `pas load --source fedstat --max-requests 5` | Изменить лимит запросов к fedstat за один запуск. |
